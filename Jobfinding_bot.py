...
class JobFindingBot:
    def __init__(self, api_key: str, serpapi_key: str, model_name: str = "meta-llama/Llama-3.3-70B-Instruct-Turbo-Free"):
        ...
        # Initialize LangChain's ChatOpenAI with Together.ai endpoint
        self.llm = ChatOpenAI(
            api_key=api_key,
        ...
        # Editable and non-editable variables
        self.variables = {
            "editable": {
        ...
        # Define tools using LangChain's StructuredTool
        self.tools = [
        ...
        # Conversation history as a list of LangChain message objects
    ...
    def chat(self, user_input: str):
        ...
        try:
            ...
            if response.tool_calls:
                self.conversation_history.append(response)  # Append assistant message with tool calls
...
def test_job_bot():
    TOGETHER_API_KEY = "0e2df1900707fffbc6c0a7254e6f091e707f69aa68094bb1fa4f7b6c1d484bcb"
    SERPAPI_API_KEY = "f00134534c7689794bf8bfc99acbc7c922b2b9c0c61f641953062ad2bb16b699"
    ...
    queries = [
        "Search DevOps jobs in Noida and set a follow-up reminder for Apple and also set a meeting with harsh his mail is harsh@gmail.com.",
        "I'm looking for data analyst roles around New York with at least 2 years experience required.",
        "Any remote openings for Python developers with fintech companies?",
        "Find me marketing jobs that offer more than ₹70,000 per month.",
        "I want something entry-level in content writing, preferably in Bangalore.",
        "Can you check if there are openings for UI/UX designers in Pune with hybrid mode?",
        "I have 5 years of experience in Java and Spring Boot. What roles would fit me?",
        "I've worked at Infosys and Paytm — show me similar companies hiring.",
        "Can you list all my preferences so far?",
        "Summarize my skills and tell me what roles I’m best suited for.",
        "Book an interview for me on 3rd June at 11am with recruiter@abc.com.",
        "Apply for all React roles in Hyderabad and schedule a follow-up email by next Friday.",
        "Set a reminder to check on Amazon job listings every Monday.",
        "Actually, I'm fine with relocating to Chennai too.",
        "Update my expected salary to 18 LPA and remove HR roles from suggestions.",
        "Search DevOps jobs in Noida and set a follow-up reminder for Apple."
    ]
    ...
    for query in queries:
        ...
        print("-"*50)
    print(f"\nUpdated Variables: {json.dumps(bot.variables['editable'], indent=2)}")
